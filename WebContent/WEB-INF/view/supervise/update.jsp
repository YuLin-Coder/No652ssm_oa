...
<meta charset="UTF-8">
<meta name="viewport"
	content="width=device-width, SuperviseDate-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
					...
					<div class="main">
						...
						<div class="form-box">
							<div class="row">
								<div class="col-5">
									<form id="saveForm">
									<input type="hidden" name="id" value="${SuperviseDate.id }"> 
										<div class="form-group">
											<label>员工</label> <select name="uid" class="form-control">
												<c:forEach items="${userList}" var="data">
													<option value="${data.id}" <c:if test="${SuperviseDate.uid == data.id}">selected</c:if> >${data.realName}</option>
												</c:forEach>
									    ...
									    <div class="form-group">
											...
											<input type="text" name="times" id="times" value="${SuperviseDate.times }"  class="form-control" autocomplete="off"   onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd'})"> 
												<small class="form-text text-muted"></small>
										</div>
										<div class="form-group">
										   <label>督办类型</label> 
										   <select name="types" class="form-control">
													<option value="表扬" <c:if test="${SuperviseDate.types == '表扬'}">selected</c:if>>表扬</option>
													<option value="批评" <c:if test="${SuperviseDate.types == '批评'}">selected</c:if>>批评</option>
											</select> 
											<small class="form-text text-muted"></small>
										</div>

										<div class="form-group">
											<label>督办内容</label> <input type="text" name="content" value="${SuperviseDate.content }"
						...
						<div class="submit-box">
							...
							<a type="button" href="SuperviseList" class="btn btn-outline-secondary">返回</a>
		...
		$("#save").click(function() {
			$.ajax({
				...
				success : function(e) {
					if (e) {
						alert("修改成功！");
						window.parent.location.href = "SuperviseList";
					} else {
						alert("修改失败！");
					}
				}
